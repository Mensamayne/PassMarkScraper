...
def extract_component_name(soup: BeautifulSoup, url: str) -> str:
    ...
    for tag, attrs in selectors:
        ...
        if element:
            ...
            if (
                name
                and len(name) > 3
                and not any(x in name.lower() for x in ["benchmark", "list", "chart", "rating"])
            ):
                return name
    ...
    if title:
        ...
        title_text = re.sub(
            r"\s*-\s*(PassMark|CPU|GPU|Benchmarks?).*", "", title_text, flags=re.IGNORECASE
        )
...
def extract_passmark_score(soup: BeautifulSoup, component_type: str) -> int:
    ...
    score_elements = soup.find_all(
        ["span", "strong", "div"], class_=re.compile(r"(mark|score|rating)", re.IGNORECASE)
    )
